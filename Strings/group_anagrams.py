...
def groupAnagrams1(words):
    ...
    while idx < len(sorted_idxs):
        ...
        while idx < len(sorted_idxs) and \
                sorted_words[sorted_idxs[idx]] == sorted_words[sorted_idxs[idx-1]]:
...
class Solution:
    def groupAnagrams(self, strs):
        ...
        for word in strs:
            ...
            # check if we have seen the sorted word b4
            sorted_word = "".join(sorted(word))
            if sorted_word not in store:
                store[sorted_word] = [word]
            else:
                store[sorted_word] = store[sorted_word] + [word]
...
def groupAnagrams10(words):

    store = {}
    for idx, word in enumerate(words):
        sorted_letters = "".join(sorted(word))
        if sorted_letters in store:
            store[sorted_letters] = store[sorted_letters] + [idx]
        else:
            store[sorted_letters] = [idx]

    output = []
    for key in store:
        word_group = []
        for idx in store[key]:
            word_group.append(words[idx])
