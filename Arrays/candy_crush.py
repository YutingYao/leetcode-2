...
class Solution:
    ...
    def candyCrush(self, board: List[List[int]]) -> List[List[int]]:
        ...
        for x in range(h-2):
            for y in range(w):
                ...
                if abs(board[x][y]) == abs(board[x+1][y]) == abs(board[x+2][y]) != 0:
                    board[x][y] = board[x+1][y] = board[x+2][y] = -abs(board[x][y])
                    toBeCrushed = True
        ...
        for y in range(w-2):
            for x in range(h):
                ...
                if abs(board[x][y]) == abs(board[x][y+1]) == abs(board[x][y+2]) != 0:
                    board[x][y] = board[x][y+1] = board[x][y+2] = -abs(board[x][y])
                    toBeCrushed = True
        ...
        if toBeCrushed: # not needed but speeds it up
            ...
            # find negative values then,
            # replace it with the ones above it
