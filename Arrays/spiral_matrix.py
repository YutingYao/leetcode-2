...
# O(n) time | O(n) space | n = total number of elements in the input matrix
...
def spiralTraverse(array):
    ...
    while (rowBegin <= rowEnd) and (columnBegin <= columnEnd):
        ...
        for row in range(rowBegin + 1, rowEnd+1):
            result.append(array[row][columnEnd])

        for col in reversed(range(columnBegin, columnEnd)):
        ...
        for row in reversed(range(rowBegin + 1, rowEnd)):
